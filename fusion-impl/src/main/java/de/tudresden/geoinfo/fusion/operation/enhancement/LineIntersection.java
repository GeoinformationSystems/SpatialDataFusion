...
public class LineIntersection extends AbstractOperation {
	...
	private static final IIdentifier PROCESS = new Identifier(LineIntersection.class.getSimpleName());
	...
	public Map<IIdentifier,IOutputConnector> initOutputConnectors() {
		...
		outputConnectors.put(OUT_FEATURES, new OutputConnector(
				OUT_FEATURES,
				new MetadataForConnector(OUT_FEATURES.toString(), "Intersected features"),
				new IDataConstraint[]{
						new BindingConstraint(GTFeatureCollection.class),
						new MandatoryConstraint()},
				null));
